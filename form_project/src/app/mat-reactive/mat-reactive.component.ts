...
export class MatReactiveComponent implements OnInit {
  ...
  getSelectedHobby(event: any) {
    ...
    this.SelectedData =  this.selectedHobby.push(new FormControl(event.target.value));   
  ...
  fetchSelectedHobby(selectedHobby: string[]) {
    for (let sel of selectedHobby) {
      for (let fet of this.hobbies) {
        if (sel == fet['name']) {
          fet['selected'] = true;
          console.log(this.selectedHobby.push(new FormControl(fet['name'])));
        }
      }
    }
  }
  ...
  userhobbyAdded(){
    return this.addedhobby = true;
  
   }
     Added(){    
    console.log( this.hobbies.push({id: this.hobbies.length + 1,
      name: this.hobbyValue,
      selected: false,}));
     console.log(this.hobbies);

      this.addedhobby =false;
   }

   gethobby(event: Event){
